...
class ControllerExtensionModuleSofiltershopby extends Controller {
	...
	// Get data
		...
		if(count($products_arr_id) > 0)
		{
			...
			$numbers = array_column($product_data, 'price_soFilter');
			$minPrice = min($numbers);
			$maxPrice = max($numbers);
			foreach($product_data as $item)
			{
				if($item['price_soFilter'] < $minPrice)
				{
					$minPrice = $this->currency->format($item['price_soFilter'], $this->session->data['currency']);					
				}
				if($item['price_soFilter'] > $maxPrice)
				{
					$maxPrice = $this->currency->format($item['price_soFilter'], $this->session->data['currency']);
				}
			}
		}
		...
		$data['minPrice'] = $data['minPrice_new'] = round(floatval(str_replace($data['currencies'], '', $minPrice)));
		$data['maxPrice'] = $data['maxPrice_new'] = round(floatval(str_replace($data['currencies'], '', $maxPrice)));
	...
	public function filter_data(){
		...
		if(isset($_COOKIE["listingType"]) && $_COOKIE["listingType"]){
			$data['listingType'] = $_COOKIE["listingType"];
		}
        else{
        	$data['listingType'] = $data['soconfig']->get_settings('days') ? 'list' : 'grid';
        }
		ob_start();
		//include(DIR_TEMPLATE .$this->config->get('theme_default_directory') . '/template/extension/module/so_filter_shop_by/category.tpl');

		include_once(DIR_SYSTEM . 'library/template/Twig/Autoloader.php');
		Twig_Autoloader::register();
		$loader_file = new Twig_Loader_Filesystem(DIR_TEMPLATE.$this->config->get('theme_default_directory')."/template/");
		$twig = new Twig_Environment($loader_file, array('cache'=>DIR_CACHE));
		$template = $twig->loadTemplate("product/category.twig");
		$template = $twig->loadTemplate("soconfig/listing.twig");
		$data_template = array(
			'header'  => $header,
			'footer'  => $footer,
			'continue'  => $continue,
